      ...
      <footer className='bg-[#1A1C1B] h-[420px] w-full px-5 py-5 text-white mt-20'>
         ...
         <section className='flex flex-col py-10'>
          {menuoption.map((value)=>{
            return(
              <p key={value.id} className='text-[18px]'><span className='border-b'>{`${value.title} />`}</span></p>
            )
          })}
         </section>

         <section className='flex items-center justify-between pb-10'>
           <h1 className='text-[18px] font-medium'>social accounts{'->'}</h1>
            <div className='flex items-center gap-3'>
            {socialaccounts.map((value)=>{
             return(
               <h1 key={value.id} className='text-[22px]'>{value.img}</h1>
             )
           })}
            </div>
         </section>
